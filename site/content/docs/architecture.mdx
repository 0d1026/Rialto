...
icon: Layers
title: "Architecture"
description: "Components, payment and discovery flows, trust boundaries, and failure behavior."
---
...
settled testnet runs, or an accepted design decision in [design decisions](/docs/decisions/upto-onchain-cap). The contract
research spike has landed as a full spec in this repo
([`docs/scheme_upto_stellar.md`](/docs/scheme-upto-stellar)); §3.3 matches it.
...
- `POST /federation/register` / `GET /federation/peers` - a facilitator declares itself
  (`name`, `baseUrl`, `catalogUrl`) into this instance's peer table; transparent, but
  only ever a local list, and only ever populated by whoever already knows to call it.
- ingestion of external catalogs (CDP-shaped exports, AlgoVoi's live feed) is a
  **manual** CLI invocation - a direct HTTPS pull from the source, entries marked
  ...
  (see the [threat model](/docs/threat-model) §6) - a deliberate containment boundary
  today, not yet a scheduled job.
- cross-publishing (Rialto's own catalog exported in a shape others can ingest) is not
  implemented at all yet - only ingestion *into* Rialto is.
...
registries instead of one is still a registry problem. **[ADR 0003](/docs/decisions/federation-dht-discovery)**
...
catalogs pulled directly over HTTPS from the source facilitator, exactly as today's
ingest CLI already does, so there's still no relay hop and no custom message-signing
scheme required. Worked out in full, including why not a full libp2p node, on the
[Federation](/docs/federation) page. **This is a design proposal, not implemented** -
the bullets above are the accurate description of what runs today.
...
([`docs/scheme_upto_stellar.md`](/docs/scheme-upto-stellar)). The cap is enforced on-chain
...
each stage adds is visible, not blended into one number. A CI job runs it on every change
touching `packages/discovery/src/search/**` and fails the build if the fused
configuration regresses past a documented threshold against a committed baseline. Anyone
can re-run `pnpm eval` and get the same numbers, modulo the catalog's actual contents at
the time.
...
generates synthetic queries/embeddings. What's built today is 15 queries across the
required categories (exact-name, paraphrase, filtered, no-result, adversarial) against an
18-resource seed catalog, judged in a single pass by the same model that built the
harness - no separate judge model, no human audit yet. That gap is documented in the
harness's own README, not hidden. See [`/docs/benchmarks`](/docs/benchmarks) for the
