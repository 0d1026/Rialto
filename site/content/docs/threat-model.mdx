...
icon: ShieldAlert
title: Threat Model
description: "Audit-readiness deliverable: assets, trust boundaries, and threats across the facilitator, the catalog, and the discovery search surface."
---
...
- `routeTemplate` is **percent-decoded before** traversal/scheme checks, not after -
  ...
  test fixture distinct from the plain-traversal case (`test/unit/01.gauntlet.spec.ts`).
...
(including the AND→OR fallback described in [lexical search](/docs/search/lexical-bm25)),
and the dense-search cosine query. The `extensions` case in §4.1 was the one place a
user value was embedded into a *sub-language string* (jsonpath) rather than passed as
a plain SQL parameter, which is why it needed its own escaping discipline separate
from `pg`'s parameterization - worth remembering as a category if a similar
sub-language (e.g. a future full-text query syntax) gets exposed to user input later.
...
inference call per request (see [dense retrieval](/docs/search/dense-retrieval)) -
meaningfully more CPU per request than a database read. This is an open gap, not a
mitigated one; flagged here rather than implied to be handled.
...
optionally paired with `FACILITATOR_API_KEY` gating (see the
[operator guide](/docs/guides/operators)). **Not** implemented, despite being
described in an earlier draft of the facilitator's README as if it were:
per-principal cost accounting, or rate limits tied to a caller's settlement *success
rate* specifically (as opposed to raw request volume). A caller with a valid API key
(or no key at all, if the operator left it unset) can currently submit failing
settlements at the same rate as succeeding ones without the system distinguishing
between them. This is the most consequential open gap in this document - it's the one
with a direct path to operator financial loss, not just information disclosure or
availability.
...
**Not independently verified**: custom `__check_auth` smart-account support, and the
precise behavior under Soroban resource-limit exhaustion, both depend entirely on
`@x402/stellar`'s internals and were not audited as part of this document - stated as
an open item rather than assumed safe.
...
**Forward-looking, not built**: [ADR 0003](/docs/decisions/federation-dht-discovery)
proposes replacing manual peer registration with a Kademlia DHT for the discovery step
(mechanism: [Federation](/docs/federation)). Recorded here now, ahead of any
implementation, so the threat surface it would add is on the record before it exists
...
- **Poisoned DHT records** - a forged record can point a client's lookup at a
  ...
  - it runs on every ingested entry regardless of how the source peer was found,
  unchanged from the manual-registration model. A poisoned record can at worst get a
  client to *pull from* a malicious source; it cannot get malicious content *trusted*
  once pulled, any more than a malicious `catalogUrl` typed by hand into today's
  ingest CLI could.
- **Resource-pointer records** (see [Federation](/docs/federation) for the proposed
  DHT-backed query fan-out) carry the same category of risk as facilitator records,
  not a new one - a forged pointer wastes a follow-up HTTP round-trip at worst, since
  the response still has to pass the gauntlet. The one new wrinkle: these records are
...
The `upto` scheme's Soroban contract has no implementation yet (spec only, see the
[upto scheme spec](/docs/scheme-upto-stellar)) - nothing to threat-model until it
exists. The MCP server and seller SDK are README-only stubs, same reasoning. This
document will need a real revision once either lands, not just an addendum.
