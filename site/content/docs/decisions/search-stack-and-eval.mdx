---
icon: Search
title: "ADR 0002: Search stack and evaluation"
description: "Hybrid retrieval on PostgreSQL with a public evaluation harness."
---
...
- **`vector`, not `vector(n)`.** pgvector's embeddings column is deliberately dimension-
  ...
  share a dimension by construction.
...
- **Synthetic queries are template-based, not LLM-authored.** No query-generation LLM was
  available in the environment this was built in. A documented, fixed set of natural-
  language-ish questions is generated per resource from its own metadata. This is the
  piece most worth upgrading once LLM access exists for it - same gap as the judging side.
- **Eval harness is v1-scoped, not yet at target.** 15 queries (not 100-200) against an
  ...
  [`/docs/benchmarks`](/docs/benchmarks) for the current numbers.
