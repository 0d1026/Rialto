...
selection** (see [Federation](/docs/federation) for the mechanism). A search still
answers from the local catalog first; fan-out additionally sends it live to known
peers, merges results, and is honest (`partialResults: true`) about anything skipped.
Whichever peer's answer scores best gets a resource-pointer record published back
into the DHT, so the next node needing that same resource resolves it by lookup
instead of repeating the fan-out. This is federated-IR "resource selection," not a
novel mechanism, and it composes with the presence layer rather than requiring a
second protocol.
