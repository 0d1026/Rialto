---
icon: GitMerge
title: "Fusion, structured constraints, and settlement-history ranking"
description: "How the lexical and dense arms combine - reciprocal rank fusion, hard filters vs. ranking boosts, and settlement history as a tiebreak, never an override."
---
...
<Mermaid chart={`flowchart TD
    ...
    SC -->|stripped text| L["lexical arm<br/>lexical.ts + bm25.ts"]
    SC -->|stripped text| D["dense arm<br/>dense.ts"]
...
- **Lexical** = one arm. Keyword matching only, via Postgres FTS + BM25F ([its own
  page](/docs/search/lexical-bm25)). Can be called on its own (`catalog.search()` -
  the "BM25-only" configuration in the eval harness).
- **Dense** = the other arm. Embedding/cosine-similarity matching only, no keywords at
  all ([its own page](/docs/search/dense-retrieval)). Also callable on its own
  (`catalog.denseOnlySearch()` - "dense-only").
...
- **Hybrid** = the whole pipeline in the diagram above, start to finish. This is what
  ...
  of the four that produces `partialResults` (§5) - because it's the only one that
...
order (§2 explains why an empty ranker contributes nothing, not a penalty). What can
look like "the whole search is broken" during debugging is often "lexical alone, with
an empty second input" - a distinction that only makes sense once these four words are
kept separate.
...
not tuned against this catalog. This is a live, open question in the
[benchmarks](/docs/benchmarks): at only 18 resources, a smaller `k` (weighting top
ranks more aggressively) might behave better than the literature default tuned for
...
from `resources.settlement_count` (atomically incremented on every settlement, not
read-modify-write - see the [architecture doc](/docs/architecture)), applied only
where it can't override anything more informative.
