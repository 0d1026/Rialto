...
export function createRouter(
  ...
  router.get("/discovery/resources", async (req: Request, res: Response) => {
    ...
    const { items, total } = await catalog.list({
      ...
      scheme:
        typeof req.query.scheme === "string" ? req.query.scheme : undefined,
      network:
  ...
  router.get("/discovery/search", async (req: Request, res: Response) => {
    ...
    const { items, total, partialResults } = await catalog.hybridSearch(
      opts.embeddingModel,
      query,
      {
        type: typeof req.query.type === "string" ? req.query.type : undefined,
    ...
    res.json({
      ...
      partialResults,
      pagination:
        nextOffset < total ? { limit, cursor: encodeCursor(nextOffset) } : null,
    });
  });
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      if (opts.ingestToken) {
        ...
        if (provided !== opts.ingestToken) {
          res
            .status(401)
            .json({
              error: { code: "payload_invalid", reason: "bad ingest token" },
            });
          return;
        }
      }
      ...
      res
        .status(202)
        .json({ accepted: true, cataloged: true, dropped: result.dropped });
    },
  );
