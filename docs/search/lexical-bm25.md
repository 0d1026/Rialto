...
from doing so. Semantic understanding is [dense retrieval's](dense-retrieval.md) job,
not this layer's. What lexical search has to get right, on its own terms, is: given
that a document contains the query's words, which document is *more* relevant, and
why.
...
- **Candidate selection**: which rows are even in the running for a query (§3 below).
...
didn't exist yet when they were set - `packages/eval-harness` (see
[evaluation-methodology.md](evaluation-methodology.md)) is what would justify retuning
...
## 5. What this section does not cover

Query-derived structured constraints (network/asset/price), combining this arm with
[dense retrieval](dense-retrieval.md), and settlement-history ranking are covered in
[fusion-and-ranking.md](fusion-and-ranking.md) - none of that logic lives in `bm25.ts`
or `lexical.ts`, deliberately: this layer's only job is "given these words, in this
corpus, which candidate is more relevant," nothing about *which* candidates are even
eligible beyond text matching.
